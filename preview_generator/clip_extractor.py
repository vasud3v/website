...
class ClipExtractor:
    ...
    def extract_clip(
        ...
        """
        Extract a single clip with quality optimization
        ...
        try:
            cmd = [
                ...
                '-i', self.video_path,
                '-t', str(duration),
                '-vf', f'scale=-2:{resolution},fps={fps}',
                '-c:v', 'libx264',
                '-preset', 'fast',
                '-crf', str(crf),
                '-c:a', 'aac',
    ...
    def extract_multiple_clips(
        ...
        if parallel and len(timestamps) > 1:
            ...
            for i, (start_time, duration) in enumerate(timestamps, 1):
                ...
                tasks.append((i, start_time, duration, output_path, resolution, crf, fps))
    ...
    def _extract_clip_worker(task, video_path):
        ...
        Args:
            task: Tuple of (index, start_time, duration, output_path, resolution, crf, fps)
            video_path: Path to source video
        
        Returns:
            Tuple of (success, output_path)
        """
        i, start_time, duration, output_path, resolution, crf, fps = task
