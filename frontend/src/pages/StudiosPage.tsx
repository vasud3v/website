...
export default function StudiosPage() {
    ...
    if (!data || data.length === 0) return (
        <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
            <img src="/logo-icon.svg" alt="Javcore" className="w-12 h-16 opacity-50" />
            <p className="text-center text-muted-foreground">No studios found</p>
        </div>
    );

    return (
        <div className="w-full max-w-[1280px] mx-auto px-4 py-6">
            <h1 className="text-2xl font-bold mb-6 text-foreground dark:text-white">Studios</h1>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {data.map((studio) => (
                    ...
                    >
                        <h3 className="font-semibold mb-2 text-card-foreground dark:text-white">{studio.name}</h3>
                        <p className="text-sm text-muted-foreground dark:text-white/60">{studio.video_count} videos</p>
                    </Link>
                ))}
            </div>
        </div>
