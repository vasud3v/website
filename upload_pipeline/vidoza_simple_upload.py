"""
Simple upload script for Vidoza
...
DATABASE_PATH = "../database/vidoza_host.json"
...
def upload_video(video_path, title=None):
    ...
    if not all([EMAIL, PASSWORD, API_KEY]):
        print("✗ Vidoza credentials not found in .env file")
        print("  Required: VIDOZA_EMAIL, VIDOZA_PASSWORD, VIDOZA_API_KEY")
        return False
    ...
    print("=" * 70)
    print("VIDOZA VIDEO UPLOADER")
    print("=" * 70)
    print()
    
    # Upload
    uploader = VidozaUploader(EMAIL, PASSWORD, API_KEY)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python vidoza_simple_upload.py <video_path> [title]")
        print("\nExamples:")
        print("  python vidoza_simple_upload.py ../test.mp4")
        print("  python vidoza_simple_upload.py ../video.mp4 \"My Video Title\"")
        sys.exit(1)
