...
export default function Categories() {
  ...
  const { data: categories, loading } = useCachedApi<Category[]>(
    () => api.getCategories(),
    { cacheKey: 'categories:all', ttl: CACHE_TTL.LONG }
  );
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          >
            <Grid3X3 className="w-6 h-6" style={{ color: color.hex }} />
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">Categories</h1>
            <p className="text-white/50 text-sm">{filtered.length} categories</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        {paginated.map((category) => (
          ...
          >
            ...
            <p className="text-white/40 text-xs mt-1">{category.video_count} videos</p>
          </button>
        ))}
      </div>
