...
export default function StudioVideos() {
  ...
  const { data, loading, error } = useCachedApi<PaginatedResponse<VideoListItem>>(
    () => api.getVideosByStudio(name!, page, 20),
    { cacheKey: `studio:${name}:${page}`, ttl: CACHE_TTL.MEDIUM, skip: !name },
    [name, page]
  );
  ...
  if (!name) {
    return (
      <div className="container mx-auto px-4 py-6">
        <p className="text-white/60">Studio not found.</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <Building2 className="w-7 h-7" style={{ color: color.hex }} />
          </div>
          <div>
