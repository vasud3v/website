...
def extract_code_from_url(url):
    ...
    if match:
        ...
        if re.match(r'^[A-Z0-9_-]+$', code) and len(code) > 0:
            ...
            print(f"  [i] Generated code for Japanese-only title: {generated_code}")
...
test_urls = [
    ...
    ("https://javmix.tv/video/%E3%82%B9%E3%83%AC%E3%83%B3%E3%83%80%E3%83%BC%E7%BE%8E%E5%A5%B3%E3%81%AE%E7%B8%A6%E6%92%AE%E3%82%8A%E9%A8%8E%E4%B9%97%E4%BD%8D%EF%BC%8FSHO", "JPN-"),  # Will generate hash
...
for url, expected in test_urls:
    ...
    # For generated codes, just check if it starts with expected prefix
    if expected.startswith("JPN-"):
        status = "✓" if result.startswith("JPN-") else "✗"
        if result.startswith("JPN-"):
            passed += 1
        else:
            failed += 1
    else:
